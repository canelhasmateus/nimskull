discard """"
description: "results in a strange error message Error: 'result.data.bytes.bytes[result.data.bytes.ibegin + result.data.ibegin + i]' cannot be assigned to The problem goes away if the converter definition is removed or if the []= template is changed to a regular proc. Works if you make BytesRange a ref object"
"""
